...
The dashboard SHALL display the ESP32-CAM video stream in the Tracking mode view. The stream SHALL use the compressed image topic `/camera/image_raw/compressed` to minimize bandwidth. The video SHALL render to a Canvas element at the camera's native resolution. The Canvas element SHALL have an accessible label.
