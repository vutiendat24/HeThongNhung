...
## Requirement: Shared Constants in config.h

Firmware-wide constants SHALL be defined once in `config.h`. Duplicate constant definitions across modules SHALL be consolidated into `config.h`.

### Scenario: SCAN_POINTS defined once

- **WHEN** inspecting `lidar.cpp` and `ros_bridge.cpp`
- **THEN** neither file defines `SCAN_POINTS` locally; both reference the single definition in `config.h`

### Scenario: LED_STATUS_PIN defined once

- **WHEN** inspecting `main.cpp` and `ros_bridge.cpp`
- **THEN** neither file defines the LED pin constant locally; both reference `LED_STATUS_PIN` from `config.h`
