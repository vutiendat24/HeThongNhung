...
The dashboard SHALL allow users to set navigation goals by clicking on the map. Clicking SHALL send a goal to the `/navigate_to_pose` action server. The goal pose orientation SHALL default to facing the click direction from current robot position. Navigation goal state SHALL be shared between goal-setter and status components via a singleton action client.
